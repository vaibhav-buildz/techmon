        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full bg-background border border-border px-4 py-2.5 rounded-lg text-sm text-heading font-heading font-medium focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/15 transition-all"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                placeholder="Type technology (e.g. Next.js, Rust, PostgreSQL) & press Enter"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="block text-[11px] font-mono font-bold uppercase tracking-wider text-heading flex items-center gap-1.5">
                <GitBranch className="w-3.5 h-3.5 text-muted" /> GitHub Repository
              </label>
              <input
                type="url"
                value={githubUrl}
          ...
          <div className="space-y-2 pt-2 border-t border-border">
            ...
            {imageUrl ? (
              <div className="relative w-full h-44 border border-border bg-background rounded-xl overflow-hidden group shadow-inner">
                <img src={imageUrl} alt="Project Preview" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-end p-3">
                  <button
                    type="button"
                    onClick={() => setImageUrl("")}
                    className="p-2 bg-black/80 text-white rounded-full hover:bg-red-600 transition-colors shadow-lg active:scale-95"
                    title="Remove image"
                  >
                    <X className="w-4 h-4" />
                  </button>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3">
                <label className="w-full sm:w-auto flex-1 cursor-pointer flex items-center justify-center gap-2.5 border border-dashed border-border/90 hover:border-accent p-4 rounded-xl bg-background hover:bg-surface transition-all text-xs font-mono text-muted group">
                  {uploadingImage ? (
                    <Loader2 className="w-4 h-4 animate-spin text-accent" />
                  ) : (
                    <Upload className="w-4 h-4 text-accent group-hover:scale-110 transition-transform" />
                  )}
                  <span className="group-hover:text-heading transition-colors">{uploadingImage ? "Uploading..." : "Upload Cover Image"}</span>
                  <input
                    type="file"
                    accept="image/*"
                ...
                <div className="text-[11px] font-mono text-muted uppercase">OR</div>
                <input
                  type="url"
                  value={imageUrl}
